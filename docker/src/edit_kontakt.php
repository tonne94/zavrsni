...
<table style="width:1000" border="0" align="center" >
	...
	<?php 
		...
		else if (isset($_POST ['kontakt']))
		{
			...
			$ID_korisnika = $row[0];						$query = "UPDATE `korisnik` SET `kontakt` = '$kontakt' WHERE `korisnik`.`ID_korisnika` = '$ID_korisnika'; ";
			mysql_query($query);
			$query = "SELECT * FROM korisnik WHERE ID_korisnika = '$ID_korisnika' AND kontakt = '$kontakt'";
			$result = mysql_query($query);
			$row_check = mysql_fetch_array($result, MYSQL_ASSOC);
			mysql_free_result($result);
			if ($row_check)
			{
				// uspjesno upisivanje u baz
				print ("Uspješno ste se promijenili kontakt! Kliknite <a href='profile.php'>ovdje</a> za povratak na profil.");
			} 
			else 
			{
